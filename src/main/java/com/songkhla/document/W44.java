...
//			table1.add(repl2);
		...
		try {
			...
			wordMLPackage.save(new java.io.File("C:/สำนวนอิเล็กทรอนิกส์"+"/"+PoliceStationName+"/ปี"+ccYear+"/"+casetype+"/"+casetype+cs+"-"+ccYear+"/คำร้องขอคืนสิ่งของ" +cs+"-"+ccYear+".doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
            }
...
//              
		...
		try {
			...
			wordMLPackage.save(new java.io.File("C:/สำนวนอิเล็กทรอนิกส์/แบบฟอร์มสำนวน/คำร้องขอคืนสิ่งของ.doc"));
		}catch( Exception ex) {
			ex.printStackTrace();
		}
            }
